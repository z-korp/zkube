...
export const ProgressBar = ({
  ...
  const fillColor = isDanger ? color.status.danger : color.accent.blue;
  ...
  const tickCallback = useCallback((ticker: { deltaMS: number }) => {
    ...
    g.fill({ color: color.text.primary, alpha: 0.15 });
  ...
  const drawBackground = useCallback((g: PixiGraphics) => {
    ...
    g.roundRect(0, 0, width, barHeight, cornerRadius);
    g.stroke({ color: color.bg.primary, width: 1, alpha: 0.5 });
