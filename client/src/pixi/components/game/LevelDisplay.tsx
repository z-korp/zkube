...
// Component-local color constants
const STAR_GLOW = 0xfef3c7;
const STAR_HIGHLIGHT = 0xfcd34d;
const SUCCESS_BORDER = 0x4ade80;
const INCOMPLETE_BG = 0x374151;
const INCOMPLETE_BORDER = 0x6b7280;
...
export const LevelDisplay = ({
  ...
  const drawLevelBadge = useCallback((g: PixiGraphics) => {
    ...
    g.fill({ color: color.bg.primary, alpha: 0.95 });
    
    g.roundRect(0, 0, levelBadgeWidth, levelBadgeHeight, 8);
    g.stroke({ color: color.accent.blue, width: 2, alpha: 0.8 });
    
    // Inner glow effect
    g.roundRect(2, 2, levelBadgeWidth - 4, levelBadgeHeight - 4, 6);
    g.stroke({ color: color.text.primary, width: 1, alpha: 0.1 });
  ...
  const drawStars = useCallback((g: PixiGraphics) => {
    ...
    const filledColor = color.accent.gold;
    const emptyColor = color.state.hover;
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (let j = 0; j < points * 2; j++) {
       ...
       if (filled && starGlowIntensity > 0) {
         ...
         g.poly(glowPath);
         g.fill({ color: STAR_GLOW, alpha: starGlowIntensity * 0.4 });
       }
      ...
      g.fill({ color: filled ? filledColor : emptyColor, alpha: filled ? 1 : 0.3 });
       ...
       if (filled) {
         g.poly(path);
         g.stroke({ color: STAR_HIGHLIGHT, width: 1, alpha: 0.5 + starGlowIntensity * 0.5 });
       }
    }
  ...
  const drawConstraintBadge = useCallback((g: PixiGraphics, _constraint: ConstraintData, isComplete: boolean) => {
    ...
    const bgColor = isComplete ? color.status.success : INCOMPLETE_BG;
    const borderColor = isComplete ? SUCCESS_BORDER : INCOMPLETE_BORDER;
