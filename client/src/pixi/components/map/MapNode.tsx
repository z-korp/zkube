...
import { getThemeColors, FONT_BOLD } from '../../utils/colors';
...
const SHOP_W = 48;
const SHOP_H = 40;
const STAR_YELLOW = color.accent.gold;
const STAR_GRAY = color.state.disabled;
// Node state colors
const STATE_COLORS: Record<NodeState, { fill: number; border: number; alpha: number; textAlpha: number }> = {
  locked:    { fill: 0x374151, border: color.state.disabled, alpha: 0.5, textAlpha: 0.4 },
  cleared:   { fill: 0x166534, border: color.status.success, alpha: 1, textAlpha: 1 },
  current:   { fill: 0x1e3a5f, border: color.accent.blue, alpha: 1, textAlpha: 1 },
  available: { fill: color.bg.primary, border: color.accent.orange, alpha: 1, textAlpha: 1 },
  ...
  const labelStyle = useMemo(
    () =>
      new TextStyle({
        ...
        fontSize: 11,
        fontWeight: 'bold',
        fill: node.state === 'cleared' ? 0x4ade80 : color.text.primary,
      }),
    [node.state],
  );
...
const BossNode = ({ node, x, y, onTap, entryDelay = 0 }: MapNodeProps) => {
    ...
    (ticker: { deltaMS: number }) => {
      ...
      if (ring) {
        ...
        for (let i = 0; i < segments; i += 2) {
          ...
          ring.stroke({ color: color.accent.gold, width: 2, alpha: colors.alpha * 0.5 });
          ring.moveTo(0, 0);
        }
      }
  ...
  const drawGlow = useCallback(
    (g: PixiGraphics) => {
      ...
      g.fill({ color: color.accent.gold, alpha: 0.3 });
    },
    [],
  );
  ...
  const drawNode = useCallback(
    (g: PixiGraphics) => {
      ...
      if (node.state === 'cleared') {
        g.circle(0, 0, r - 5);
        g.stroke({ color: color.accent.gold, width: 1.5, alpha: 0.5 });
      }
    },
  ...
  const labelStyle = useMemo(
    () =>
      new TextStyle({
        ...
        fontSize: 13,
        fontWeight: 'bold',
        fill: node.state === 'cleared' ? color.accent.gold : color.text.primary,
      }),
    [node.state],
  );
...
const ShopNode = ({ node, x, y, onTap, entryDelay = 0 }: MapNodeProps) => {
  ...
  const drawNode = useCallback(
    (g: PixiGraphics) => {
      ...
      g.fill({ color: colors.fill, alpha: colors.alpha });
      g.roundRect(-w / 2, -h / 2, w, h, 10);
      g.stroke({ color: color.status.success, width: 2, alpha: colors.alpha });
    },
    [pressed, colors],
  );
