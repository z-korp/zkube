...
import { getThemeColors, THEME_META, FONT_BOLD, FONT_BODY, FONT_TITLE } from '../../utils/colors';
...
const DIFFICULTY_LABELS: Record<string, { label: string; color: number }> = {
  ...
  Easy:       { label: 'Easy',        color: color.status.success },
  Medium:     { label: 'Medium',      color: color.accent.gold },
  MediumHard: { label: 'Medium Hard', color: color.accent.orange },
  Hard:       { label: 'Hard',        color: color.status.danger },
  VeryHard:   { label: 'Very Hard',   color: 0xdc2626 },
  Expert:     { label: 'Expert',      color: 0xb91c1c },
  Master:     { label: 'Master',      color: color.status.dangerDark },
};
...
export const LevelPreview = ({ node, screenWidth, screenHeight, isGameOver = false, onPlay, onClose }: LevelPreviewProps) => {
  ...
  const drawPanel = useCallback(
    (g: PixiGraphics) => {
      ...
      g.fill({ color: color.bg.secondary, alpha: 0.95 });
      g.roundRect(0, 0, PANEL_W, panelH, PANEL_RADIUS);
  ...
  const drawPlayBtn = useCallback(
    (g: PixiGraphics) => {
      ...
      g.fill({ color: color.accent.orange });
      g.roundRect(ox, oy, w, h, 10);
  ...
  const titleStyle = useMemo(() => new TextStyle({ fontFamily: FONT_TITLE, fontSize: 18, fill: color.text.primary }), []);
  const labelStyle = useMemo(() => new TextStyle({ fontFamily: FONT_BODY, fontSize: 11, fill: color.text.secondary }), []);
  const valueStyle = useMemo(() => new TextStyle({ fontFamily: FONT_BOLD, fontSize: 13, fontWeight: 'bold', fill: color.text.primary }), []);
  const closeBtnTextStyle = useMemo(() => new TextStyle({ fontSize: 14, fill: color.text.secondary }), []);
  const playBtnTextStyle = useMemo(() => new TextStyle({ fontFamily: FONT_BOLD, fontSize: 16, fontWeight: 'bold', fill: 0xffffff }), []);
  const starFilledStyle = useMemo(() => new TextStyle({ fontFamily: FONT_BOLD, fontSize: 14, fill: color.accent.gold }), []);
  const starEmptyStyle = useMemo(() => new TextStyle({ fontFamily: FONT_BOLD, fontSize: 14, fill: color.state.disabled }), []);
            ...
            {(isCleared || isDeathLevel) && (
              <pixiContainer>
                ...
                <pixiText
                  ...
                  y={lineH}
                  anchor={{ x: 1, y: 0 }}
                  style={new TextStyle({ fontFamily: FONT_BOLD, fontSize: 13, fontWeight: 'bold', fill: isDeathLevel ? color.status.danger : color.status.success })}
                  eventMode="none"
                />
                {hasStars && (
                  <pixiContainer>
