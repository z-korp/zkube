...
const SECTION_BORDER = color.bg.surface;
const GOLD = color.accent.gold;
const TEXT_DIM = color.text.secondary;
...
export const InGameShopModal = ({
  ...
  const bonusNameStyle = useMemo(
    () => new TextStyle({ fontFamily: FONT_BOLD, fontSize: 12, fill: color.text.primary }),
    [],
  );
  ...
  const unallocStyle = useMemo(
    () => new TextStyle({ fontFamily: FONT_BOLD, fontSize: 11, fill: color.accent.orange }),
    [],
  );
  ...
  const drawSwapPickerBg = useCallback(
    (g: PixiGraphics) => {
      ...
      g.fill({ color: color.bg.primary, alpha: 0.85 });
      g.roundRect(0, 0, sectionWidth - 20, h, 6);
