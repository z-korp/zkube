...
const SELECTED_BORDER = 0x4ade80;
const LOCKED_BORDER = 0x6b7280;
const LOCKED_TEXT = 0x9ca3af;

const LOCK_ICON_STYLE = { fontSize: 24 };
const SLIDER_VALUE_STYLE = { fontFamily: FONT_TITLE, fontSize: 18, fill: color.text.primary };
...
const CUBE_SECTION_STYLE = { fontFamily: FONT_TITLE, fontSize: 18, fill: color.text.primary };
...
const BonusTile = ({
  ...
  const bgColor = isSelected ? color.status.success : isLocked ? color.state.disabled : color.bg.primary;
  const borderColor = isSelected ? SELECTED_BORDER : isLocked ? LOCKED_BORDER : color.state.hover;
  const scale = pressed ? 0.95 : hovered && !isLocked ? 1.05 : 1;
  const bonusLabelStyle = useMemo(() => ({
    fontFamily: FONT_BODY,
    fontSize: 12,
    fill: isLocked ? LOCKED_TEXT : color.text.primary,
  }), [isLocked]);
  ...
  const draw = useCallback(
    (g: PixiGraphics) => {
      ...
      const r = 14;
      // Shadow
      g.setFillStyle({ color: color.bg.overlay, alpha: 0.3 });
      ...
      // Lock overlay
      if (isLocked) {
        g.setFillStyle({ color: color.bg.overlay, alpha: 0.5 });
        g.roundRect(0, 0, size, size, r);
        g.fill();
      }
    },
...
const CubeSlider = ({
  ...
  const drawTrack = useCallback(
    (g: PixiGraphics) => {
      g.clear();
      // Background track
      g.setFillStyle({ color: color.bg.surface });
      g.roundRect(0, knobRadius - trackHeight / 2, width, trackHeight, 5);
      g.fill();
      // Filled portion
      if (fillWidth > 0) {
        g.setFillStyle({ color: color.accent.blue });
  ...
  const drawKnob = useCallback(
    (g: PixiGraphics) => {
      g.clear();
      g.setFillStyle({ color: color.text.primary });
      g.circle(0, 0, knobRadius);
      g.fill();
      g.setStrokeStyle({ width: 3, color: color.accent.blue });
      g.circle(0, 0, knobRadius);
      g.stroke();
    },
    [knobRadius]
