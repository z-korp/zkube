...
const VolumeSlider = ({
  ...
  const drawTrack = useCallback(
    (g: PixiGraphics) => {
      g.clear();
      g.setFillStyle({ color: color.bg.surface, alpha: 1 });
      g.roundRect(0, 0, trackW, trackH, trackH / 2);
      g.fill();
      const fillW = trackW * clampedValue;
      if (fillW > 0) {
        g.setFillStyle({ color: color.text.primary, alpha: 1 });
      ...
      g.fill();
      g.setStrokeStyle({ width: 2, color: color.accent.blue, alpha: 0.8 });
      g.circle(knobX, knobCY, knobR);
      g.stroke();
    },
  ...
  const labelStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(16 * s), fill: color.text.primary }), [s]);
  const pctStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(13 * s), fill: color.text.secondary }), [s]);
...
const SettingRow = ({
  ...
  const padX = Math.round(16 * s);
  const labelStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(14 * s), fill: color.text.secondary }), [s]);
  const valueStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(14 * s), fill: color.text.primary }), [s]);
...
const ThemeOption = ({
  ...
  const drawBg = useCallback(
    (g: PixiGraphics) => {
      g.clear();
      g.setFillStyle({ color: selected ? color.accent.blue : color.bg.secondary, alpha: selected ? 0.35 : 0.01 });
      g.roundRect(0, 0, width, height, 8);
      g.fill();
      g.setStrokeStyle({ width: selected ? 2 : 1, color: selected ? 0x60a5fa : color.state.hover, alpha: selected ? 1 : 0.5 });
      g.roundRect(0, 0, width, height, 8);
      g.stroke();
    },
...
const SectionPanel = ({
  ...
  const drawBg = useCallback(
    (g: PixiGraphics) => {
      g.clear();
      g.setFillStyle({ color: color.bg.secondary, alpha: 0.92 });
      g.roundRect(0, 0, width, height, radius);
      g.fill();
      g.setStrokeStyle({ width: 1, color: color.bg.surface, alpha: 0.6 });
      g.roundRect(0, 0, width, height, radius);
      g.stroke();
    },
    [width, height, radius],
  ...
  const titleStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(16 * s), fill: color.text.primary }), [s]);
  const padX = Math.round(16 * s);
...
export const SettingsPage = ({
  ...
  const versionStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(12 * s), fill: color.state.hover }), [s]);
  const footerStyle = useMemo(() => ({ fontFamily: FONT_TITLE, fontSize: Math.round(11 * s), fill: color.state.hover }), [s]);
