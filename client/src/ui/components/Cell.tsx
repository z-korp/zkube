...
const Cell: React.FC<CellProps> = ({
  ...
  const getCellStyle = () => {
    ...
    const pieceWidth = cell.pieceId;
    const cellWidth = gridRef.current
      ? gridRef.current.getBoundingClientRect().width / cols
      : 0;
    const width = `${cellWidth * pieceWidth}px`;

    let left = "0px";
    if (isDragging) {
      const dragOffset = draggingPiece.currentX - draggingPiece.startX;
      left = `${dragOffset}px`;
    }
  ...
  return (
    <div
      className={`relative ${isLineComplete ? "animate-pulse" : ""}`}
      onClick={() => {
        handleRowClick(rowIndex);
        handleCellClick(rowIndex, colIndex);
      }}
