      ...
      <Card className="p-4 bg-slate-800">
        <div className="grid grid-cols-6 gap-1">
          {grid.map((row, rowIndex) =>
            row.map((cell: any) => (
              <div key={cell.id} className="w-12 h-12 bg-slate-700 flex items-center justify-center">
                {cell.element && <div className={`w-10 h-10 rounded-md ${getElementColor(cell.element)}`} />}
              </div>
            ))
          )}
        </div>
