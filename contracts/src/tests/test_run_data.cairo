...
#[test]
fn test_run_data_zone_id_is_4_bits_and_mutator_mask_is_u8() {
    let data = RunDataPackingTrait::new(31, 255);
    assert!(data.zone_id == 15, "zone_id should be masked to 4 bits");
    assert!(data.mutator_mask == 255, "mutator_mask should keep full u8 range");

    let unpacked = RunDataPackingTrait::unpack(data.pack());
    assert!(unpacked.zone_id == 15, "zone_id should still be 15 after roundtrip");
    assert!(unpacked.mutator_mask == 255, "mutator_mask should still be 255 after roundtrip");
}
...
fn test_run_data_zone_lifecycle_struct_manipulation() {
    ...
    let mut run_data = RunDataPackingTrait::new(1, 0);
    ...
    assert!(run_data.endless_depth == 0, "endless depth should start at 0");
    ...
    assert!(run_data.endless_depth == 1, "endless depth should be 1 on L11");
    assert_roundtrip(run_data);
}
